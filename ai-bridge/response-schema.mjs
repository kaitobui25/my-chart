...
export function normalizeTradePlan (value = {}) {
  ...
  const targets = Array.isArray(value.targets)
    ? value.targets.map(finiteOrNull).filter(item => item !== null).slice(0, 5)
    : []
  ...
  const completeTrade = entryZone !== null && stopLoss !== null && targets.length > 0
  const safeDecision = decision !== 'WAIT' && !completeTrade ? 'WAIT' : decision
